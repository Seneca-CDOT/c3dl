...
c3dl.Picking = function(scene)
{
	...
	*/
	this.onMouseDown = function(event)
	{
		...
		var normalizedDeviceCoords = [	( 2 * clickedCanvasCoords[0] / canvasTag.width) -1,
										-((2 * clickedCanvasCoords[1] / canvasTag.height) -1),
										 1,1];
		...
		if( scn.getPointRenderingMode() == c3dl.POINT_MODE_POINT)
		{
			...
			for(var i = 0; i < scn.getObjListSize(); i++)
			{
				if( scn.getObj(i) instanceof c3dl.Point )
				{
					...
					// perspective divide.
					var normalizedDeviceCoords = [	clipCoords[0]/clipCoords[3], 
													clipCoords[1]/clipCoords[3], 
													clipCoords[2]/clipCoords[3]];

					// transform the point from NDC space to viewport space.
					var viewportCoords = [ (normalizedDeviceCoords[0] +1)/2 * canvasTag.width, 
										 (1-normalizedDeviceCoords[1])/2 * canvasTag.height];
					...
					if(( scn.getPointSmooth() && isPointInCircle(clickedCanvasCoords, viewportCoords, pointPixelSize)) 
					||(	!scn.getPointSmooth() && isPointInSquare(clickedCanvasCoords, viewportCoords, pointPixelSize)))
					{
	...
	*/
	function createPickingResult(cvs, btnUsed, objList)
	{
		...
		pickingObj["getCanvas"] = function (){ return this.canvas;};
		
		//
		pickingObj["buttonUsed"] = btnUsed;
		pickingObj["getButtonUsed"] = function (){ return this.buttonUsed;};

		//
		pickingObj['objects'] = objList;
		pickingObj['getObjects'] = function(){ return this.objects;};
	...
	*/
	function isPointInSquare( pointCoords, squareCoords, squareSize)
	{
		...
		if( pointCoords[0] >= squareCoords[0] - squareSize/2 &&
			pointCoords[0] <= squareCoords[0] + squareSize/2 &&
			pointCoords[1] >= squareCoords[1] - squareSize/2 &&
			pointCoords[1] <= squareCoords[1] + squareSize/2)
		{
			return true;
		}
