...
c3dl.Effect = function()
{
	...
	*/
	...
	{		
		...
		{
			c3dl.debug.logWarning("Effect must be initialized with init() " + 
									"before setting its parameters.");
		}
		else
		{
			var isFound = false;
			...
			for(var i=0; !isFound && i < this.instanceParams.length; i++)
			{
				if(	paramName == this.instanceParams[i].name)
				{
					...
					else
					{
						...
						c3dl.debug.logWarning(	"The value '" + paramValue + "' cannot be assigned " +
												"to parameter '" + paramName + "' because it is the " +
												"incorrect type. Check the c3dl.effects documentation " +
												" for the correct type.");
					}
				}
			}
