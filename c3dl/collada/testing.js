importScripts('../c3dlnamespace.js','../math/mathutils.js','../utilities/utilities.js','../actors/actor.js', '../actors/primitive.js', '../scenegraph/scenenode.js');  
...
self.onmessage = function(event){
  ...
  parseNodeRecursive = function (xmlObject, node, sgNode)  {
    // set this node's transform
    var translateTag = getChildNodesByNodeName(node, "translate");

    // node may not have one, so check first
    if (translateTag)
    {
      // string representation of data between <translate> tags.
      var floatValues = stringsToFloats(translateTag[0].childNodes[0].nodeValue, ' ');
    ...
    // rotations
    var rotationTags = getChildNodesByNodeName(node, "rotate");
    ...
    if (rotationTags)
    {
      ...
      {
        var floatValues = stringsToFloats(rotationTags[i].childNodes[0].nodeValue, ' ');
    ...
    // <scale> tag
    var scaleTag = getChildNodesByNodeName(node, "scale");
    if (scaleTag)
    {
      var floatValues = stringsToFloats(scaleTag[0].childNodes[0].nodeValue, ' ');
    ...
    // 
    var matrixTag = getChildNodesByNodeName(node, "matrix");
    if (matrixTag)
    {
      var mat = stringsToFloats(matrixTag[0].childNodes[0].nodeValue, ' ');
    ...
    var geometries = getChildNodesByNodeName(node, "instance_geometry");
    ...
    if (geometries)
    {
      ...
      for (var currGeo = 0, len = geometries.length; currGeo < len; currGeo++)
      {
        ...
        sgNode.addChild(instantiateGeometry(xmlObject, url, geometries[currGeo]));
      }
    }
    ...
    //
    var instance_nodes = getChildNodesByNodeName(node, "instance_node");
    if (instance_nodes)
    {
      ...
      for (var currNode = 0, len = instance_nodes.length; currNode < len; currNode++)
      {
        ...
        sgNode.addChild(instantiateNode(xmlObject, url));
      }
    }
    ...
    var nodes = getChildNodesByNodeName(node, "node");
    ...
    if (nodes)
    {
      ...
      for (var i = 0, len = nodes.length; i < len; i++)
      {
        ...
        parseNodeRecursive(xmlObject, nodes[i], scenenode);
      }
    }
  }
   ...
   */
  getChoice = function (parentTag, choiceTagNames)  {
    var choice = null;
    var i = 0;
  ...
  parse = function (xmlObject)  {
    ...
    // do this, simply make a reference to the parent which was set
    ...
    var nodes = getChildNodesByNodeName(visualScene, "node");
  ...
  /**
   ...
   @param {String} target
   */
  instantiateMaterial = function (xmlObject, target)  {
    var tempTexture = null;

    // we now have the material ID which we can look up in the library materials.
    var material = findElementInLibrary(xmlObject, "library_materials", "material", target);
    var tempName = target;
    ...
    //
    //
    var effect = findElementInLibrary(xmlObject, "library_effects", "effect", instanceEffectURL);
    ...
    if (newparam)
    {
      ...
      var texture = findElementInLibrary(xmlObject, "library_images", "image", fileID);
    ...
    var shadingAlgorithm = getChoice(technique, ["blinn", "constant", "phong", "lambert"]);

    var mat = new c3dl.Material();
    //mat.texture = tempTexture;
    mat.setName(tempName);
    mat.setAmbient(getColor(shadingAlgorithm, "ambient"));
    mat.setDiffuse(getColor(shadingAlgorithm, "diffuse"));
    mat.setEmission(getColor(shadingAlgorithm, "emission"));
    mat.setSpecular(getColor(shadingAlgorithm, "specular"));
    mat.setShininess(getColor(shadingAlgorithm, "shininess"));
   ...
   */
  instantiateGeometry = function (xmlObject, url, instanceGeometryElement)  {
    var root = xmlObject.documentElement;
    ...
    for (var currColl = 0, len = collations.length; currColl < len; currColl++)
    {
      ...
      {
        var p = getFirstChildByNodeName(collations[currColl], "p");
        new C3DL_FLOAT_ARRAY(rawFaces = mergeChildData(p.childNodes).split(" "));
      }
      ...
      { /**************/
        ...
        {
          vertexOffset = inputs[i].getAttribute("offset");
          // need to remove the leading # from the value
          vertexSource = inputs[i].getAttribute("source").split('#')[1];

          var vertices = getNodeWithAttribute(xmlObject, "vertices", "id", 
            vertexSource);
          ...
          // can index into it.
          var data = getData(xmlObject, "source", "id", posSource);
          ...
          verticesArray = groupScalarsIntoArray(data.values, 3, vertexStride);
        }
        ...
        {
          normalOffset = inputs[i].getAttribute("offset");
          // need to remove the leading # from the value
          normalSource = inputs[i].getAttribute("source").split('#')[1];
          var data = getData(xmlObject, "source", "id", normalSource);
          ...
          normalsArray = groupScalarsIntoArray(data.values, 3, normalsStride);
        }
        ...
        {
          texCoordOffset = inputs[i].getAttribute("offset");
          // need to remove the leading # from the value
          var uvSource = inputs[i].getAttribute("source").split('#')[1];
          var data = getData(xmlObject, "source", "id", uvSource);
          ...
          // 1.0 1.0 0.0
          //         ^ don't need 
          texCoordsArray = groupScalarsIntoArray(data.values, 2, texCoordsStride);
        }
      }
      ...
      {
        rawFaces = splitPolylist(collations[currColl], inputs.length, rawFaces);
      }
      ...
      faces = groupScalarsIntoArray(rawFaces, inputs.length, inputs.length,collations[currColl].nodeName);
      ...
      collationElement.tempMaterial = collations[currColl].getAttribute("material");
        ...
        collationElement.init(
          expandFaces(faces, verticesArray, vertexOffset, vertexStride), 
          expandFaces(faces, normalsArray, normalOffset, normalsStride), 
          expandFaces(faces, texCoordsArray, texCoordOffset, 2)
        );
	  }
	  else {
    ...
    if (bind_material)
    {
      ...
      for (var im = 0, len2 = instance_materials.length; im < len2; im++)
      {
        ...
        //
        var matAndTex = instantiateMaterial(xmlObject, target);
   ...
   */
  instantiateNode = function (xmlObject, url)  {
    var root = xmlObject.documentElement;
    ...
    parseNodeRecursive(xmlObject, nodeToCreate, inode);
  ...
  /**
   ...
   @returns {Array}
   */
  groupScalarsIntoArray = function (rawScalarValues, numComponentsPerElement, stride)  {
   ...
   */
  splitPolylist = function (collation, numInputs, rawFaces)  {
    // rawFaces = splitPolylist(collations[currColl], inputs.length);
    // <vcount> has the count of vertices for each polygon.
    // <vcount> 4 4 4 3 4 5 3 4 .. </vcount>
    var vcountNode = getFirstChildByNodeName(collation, "vcount");
    var vcountList = mergeChildData(vcountNode.childNodes).split(" ");
  ...
  /**
   ...
   @returns
   */
  findElementInLibrary = function (xmlObject, libraryName, elementName, elementAttributeId)
  {
  ...
  /**
   ...
   @return 
   */
  getData = function (xmlObject, nodeName, attributeKey, attributeValue)
  {
    var data = new Object();

    // find node that has 'src' value as id
    var nsrc = getNodeWithAttribute(xmlObject, "source", "id", attributeValue);
    ...
    //
    var float_array = getNodeWithAttribute(xmlObject, "float_array", "id", accessorSrc);   
    // values in the DAE file are seperated with a space
    // don't use nodeValue since it will be broken up in 4096 chunks
	  data.values = new C3DL_FLOAT_ARRAY(mergeChildData(float_array.childNodes).split(" "));
    return data;
  }
   ...
   */
  expandFaces = function (faces, array, offset, numComponentsToExpand)
  {
  ...
  getColor = function (node, str)
  {
    ...
    if (component)
    {
      var value = getChoice(component, ["color", "float", "texture"]);
  ...
  /**
   ...
   @returns
   */
  mergeChildData = function (childNodes)
  {
    var values = [];
  ...
  /**
   ...
   @returns 
   */
  getFirstChildByNodeName = function (searchNode, nodeName)
  {
...
/**
 ...
 @returns 
 */
getNodeWithAttribute = function (xmlObject, nodeName, attributeKey, attributeValue)
{
  var nodeFound;
 ...
 */
getChildNodesByNodeName = function (searchNode, nodeName)
{
  var children = [];
  var foundOne = false;
 ...
 */
stringsToFloats = function (numbers, delimeter)
{
  var floatValues = [];
