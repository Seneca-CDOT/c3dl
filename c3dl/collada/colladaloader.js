...
c3dl.ColladaLoader = function()
{
	...
	*/
	this.parseNodeRecursive = function(xmlObject, node, sgNode)
	{
		...
		if(matrixTag)
		{
			var floatValues = c3dl.ColladaLoader.stringsToFloats(matrixTag[0].childNodes[0].nodeValue, ' ');
			sgNode.setTransform(c3dl.transposeMatrix(floatValues));
		}
		...
		for(var currColl = 0; currColl < collations.length; currColl++)
		{
			...
			for (var i = 0; i < inputs.length; i++)
			{
				...
				else if(inputs[i].getAttribute("semantic") == "TEXCOORD")
				{
					...
					// OpenGL ES seems to expect the bottom left corner of the
			...
			else if( collations[currColl].nodeName == "polygons")
			{
				...
				for (var currPrim = 0; currPrim < collations[currColl].getAttribute("count"); currPrim++)
				{
					...
					for (var s=0; s < partsArray[0].length; s++){trianglesList.push(partsArray[0][s]);}
					for (var s=0; s < partsArray[1].length; s++){trianglesList.push(partsArray[1][s]);}
					for (var s=0; s < partsArray[3].length; s++){trianglesList.push(partsArray[3][s]);}
					for (var s=0; s < partsArray[3].length; s++){trianglesList.push(partsArray[3][s]);}
					for (var s=0; s < partsArray[1].length; s++){trianglesList.push(partsArray[1][s]);}
					for (var s=0; s < partsArray[2].length; s++){trianglesList.push(partsArray[2][s]);}
				}
