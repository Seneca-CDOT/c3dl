...
{	
  ...
  // Create a projection matrix and store it inside a globally accessible place.
	this.projMatrix = c3dl.makePerspective(this.fieldOfView, aspectRatio, this.nearClippingPlane, this.farClippingPlane);  
...
{	
	...
	return	"c3dl.Camera: " + delimiter +
			"left: "				+ this.getLeft()			+ delimiter +
			"up: "					+ this.getUp()				+ delimiter +
			"direction: "			+ this.getDir()				+ delimiter + 
			"position: "			+ this.getPosition()		+ delimiter +
			"fied of view: "		+ this.getFieldOfView()		+ delimiter +
			"near clipping plane: "	+ this.getNearClippingPlane()	+ delimiter +
			"far clipping plane: "	+ this.getFarClippingPlane()	+ delimiter;
}
